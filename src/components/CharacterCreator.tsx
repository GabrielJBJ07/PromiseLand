    ...
    <div className="min-h-screen bg-slate-900 text-white flex flex-col justify-center items-center p-4 relative overflow-hidden">
      ...
      <div className="absolute top-6 right-6 z-20">
        <button
          ...
          className="flex items-center gap-2 bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold px-4 py-2 rounded-xl shadow-lg transition text-sm cursor-pointer"
        >
          <Sparkles className="w-4 h-4" />
          <span>초대 QR 코드 열기</span>
        </button>
      </div>

      <div className="relative z-10 w-full max-w-4xl bg-slate-800/90 border border-amber-500/30 rounded-3xl p-6 md:p-8 shadow-2xl backdrop-blur-md my-6">
        {/* Header */}
        <div className="text-center mb-8">
          <div className="inline-flex items-center gap-2 bg-amber-500/20 text-amber-300 border border-amber-500/30 px-4 py-1.5 rounded-full text-xs font-semibold mb-3">
        ...
        <form onSubmit={handleStart} className="grid grid-cols-1 md:grid-cols-12 gap-8">
          {/* Left Column: Player Info & Character Preview */}
          <div className="md:col-span-5 flex flex-col items-center bg-slate-900/80 rounded-2xl p-6 border border-slate-700/80">
            ...
            <div className="relative bg-slate-950/80 rounded-2xl border-2 border-amber-500/40 p-4 mb-4 w-44 h-44 flex flex-col items-center justify-center shadow-inner group">
              <SpriteCanvas
                ...
                direction="down"
              />
              <span className="absolute bottom-2 text-xs text-amber-300 font-medium bg-slate-900/80 px-2 py-0.5 rounded-full">
          ...
          <div className="md:col-span-7 flex flex-col justify-between">
            <div>
              <h3 className="text-sm font-bold text-slate-200 mb-3">1. 성경 인물 프리셋 선택</h3>
              <div className="grid grid-cols-4 gap-2 mb-6">
                {CHARACTER_PRESETS.map((p) => (
                  ...
                  >
                    <SpriteCanvas preset={p} size={36} animated={false} />
              ...
              <h3 className="text-sm font-bold text-slate-200 mb-3">2. 커스텀 꾸미기</h3>
              <div className="space-y-4 bg-slate-900/50 p-4 rounded-2xl border border-slate-800 mb-6">
                {/* Hair Color */}
                <div>
            ...
            <button
              type="submit"
              className="w-full bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-400 hover:to-amber-500 text-slate-950 font-black py-4 px-6 rounded-2xl shadow-xl flex items-center justify-center gap-3 text-lg transition transform hover:-translate-y-0.5 active:translate-y-0 cursor-pointer"
            >
